...
import { UserCheck, CheckCircle, XCircle, Clock, Send } from 'lucide-react';
...
export default function MarkAttendancePage() {
  ...
  const handleSubmitAttendance = () => {
    ...
    toast({
      ...
      description: `Attendance for ${mockClasses.find(c => c.id === selectedClassId)?.name || 'the class'} has been recorded. (${attendanceRecords.length} students)`,
    });
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageTitle 
        ...
        subtitle="Select a class and record daily student attendance."
      />
      <Card className="shadow-lg">
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <UserCheck className="h-5 w-5 text-primary" />
            Attendance Sheet for {new Date().toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}
          </CardTitle>
        </CardHeader>
        ...
        {selectedClassId && attendanceRecords.length > 0 && (
          <CardFooter className="border-t pt-6">
            ...
            >
              ...
              Submit Attendance for {mockClasses.find(c => c.id === selectedClassId)?.name}
            </Button>
          </CardFooter>
        )}
      </Card>
